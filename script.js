  ...
  computed: {
    ...
    totalname:function(){
      ...
      for (var i = 0; i < this.products.length; i++) {
          ...
          //if (item.count>0){s=s + item.itemName +"_____ X " + item.count +'\n'}
          //if (item.count2>0){s=s + item.itemName2 +"_____ X " + item.count2 +'\n'}
          if (item.amount>0){s=s +item.amount +"  x  " + item.name  +'\n'}
          if (item.amount2>0){s=s +item.amount2 +"  x  " + item.name2  +'\n'}
      }
      //document.getElementById("demandValue").value=s;
      return s;  
